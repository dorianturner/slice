...
<main>
  ...
  <section class="timeline-section" aria-label="Capture timeline">
    ...
    <p class="muted">Invocation bars and sampled work are shown beneath the selected time window. Drag either handle to filter the population.</p>
  ...
  <section class="flame-section"><div class="section-heading"><div><p class="label">Selected execution paths</p><h2>Flame graph</h2></div><div class="flame-actions"><label class="search"><span class="sr-only">Search frames</span><input id="frame-search" type="search" placeholder="Search frames"></label></div></div><nav id="flame-zoom-path" class="flame-zoom-path" aria-label="Flame graph zoom path"></nav><p class="muted">Hover a frame for sample count, invocation count, self time, and CPU/off-CPU breakdown. Click to zoom.</p><svg id="flame" role="img" aria-label="Interactive flame graph"></svg><div id="empty" hidden>No samples match this query.</div><div id="flame-tooltip" class="tooltip" role="status" aria-live="polite" hidden></div></section>
</main>
...
#histogram { width:100%; height:86px; display:block; margin-bottom:4px; touch-action:none; } .hist-bar { fill:#527fb0; } .hist-selected { fill:#82c4ff; } .hist-label { fill:#aeb8ca; font-size:10px; } .hist-handle { stroke:#d7efff; stroke-width:2; stroke-dasharray:3 2; pointer-events:none; } .hist-handle-hit { fill:transparent; cursor:ew-resize; } .hist-window { fill:#82c4ff; opacity:.09; cursor:grab; }
.timeline-chart-scroll { overflow-x:scroll; overflow-y:auto; scrollbar-width:auto; }
.flame-zoom-path { display:flex; align-items:center; flex-wrap:wrap; gap:5px; min-height:28px; margin:4px 0 8px; color:#8ac5ff; font-size:.76rem; } .flame-zoom-path button { padding:3px 7px; background:#202b3c; border-color:#405a78; font-size:.74rem; } .flame-zoom-path button[aria-current="page"] { color:#f4f8ff; background:#355477; } .flame-zoom-separator { color:#71829b; }
.summary { display:grid; grid-template-columns:repeat(3,1fr); gap:12px; margin:13px 0; } .summary>div { background:#1b212c; border-left:3px solid #6bbaf7; border-radius:5px; padding:12px 15px; } .summary p { color:#aeb8ca; font-size:.76rem; } .summary strong { display:block; margin-top:4px; font-size:1.2rem; font-variant-numeric:tabular-nums; } .summary span { display:block; margin-top:3px; color:#aeb8ca; font-size:.74rem; }
.flame-actions { display:flex; align-items:center; gap:8px; } .search input { width:190px; background:#202735; color:#edf0f6; border:1px solid #46536a; border-radius:5px; padding:6px 8px; } button { color:#d8e7ff; background:#28354a; border:1px solid #47617e; padding:6px 9px; border-radius:5px; cursor:pointer; } button:hover { background:#354a67; } #flame { width:100%; min-height:160px; display:block; margin-top:12px; background:#0f131a; border-radius:7px; } .frame { stroke:#111318; stroke-width:1; cursor:pointer; } .frame:hover,.frame.match { stroke:#fff; stroke-width:2; } .frame-label { fill:#11151b; pointer-events:none; font-size:11px; dominant-baseline:middle; } #empty { color:#f5c28a; padding:38px; text-align:center; } .tooltip { position:fixed; z-index:10; max-width:330px; pointer-events:none; background:#252c39; border:1px solid #59708f; border-radius:7px; box-shadow:0 8px 28px #0008; padding:10px 12px; font-size:.77rem; line-height:1.5; } .tooltip strong { color:#e6f3ff; display:block; margin-bottom:3px; } .tooltip span { color:#b9c7d9; display:block; } .sr-only { position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0; }
...
(() => {
  ...
  function paintTimeline(result) {
    ...
    [0,.25,.5,.75,1].forEach(ratio=>appendText(svg,ns((bounds.to-bounds.from)*ratio),ratio*chartWidth,axis+threadRows.length*row+1,'timeline-axis')); const begin=(kind,event)=>{drag={kind,pointerId:event.pointerId,startX:event.clientX,from:state.time.from_ns,to:state.time.to_ns,rect:svg.getBoundingClientRect()};svg.setPointerCapture(event.pointerId);event.preventDefault();}; const handle=(kind,value)=>{const x=position(value)*chartWidth,line=svgEl('line',{class:'time-handle',x1:x,x2:x,y1:0,y2:height,'data-handle':kind}),hit=svgEl('rect',{class:'time-handle-hit',x:x-8,y:0,width:16,height,'data-handle':kind});line.style.stroke='#d7efff';line.style.strokeWidth='2';hit.addEventListener('pointerdown',event=>{event.stopPropagation();begin(kind,event);});svg.appendChild(line);svg.appendChild(hit);}; handle('time-from',state.time.from_ns);handle('time-to',state.time.to_ns);
    svg.onpointerdown=event=>{const classes=event.target.classList,value=timelineValue(event,svg,width,0,chartWidth);if(!classes.contains('timeline-axis')&&!classes.contains('time-handle-hit')&&value>=state.time.from_ns&&value<=state.time.to_ns)begin('time-move',event);}; svg.onpointermove=event=>{if(!drag||drag.pointerId!==event.pointerId)return;const rect=drag.rect,value=bounds.from+Math.max(0,Math.min(1,(event.clientX-rect.left)/rect.width))*(bounds.to-bounds.from),span=bounds.to-bounds.from,delta=(event.clientX-drag.startX)/rect.width*span;if(drag.kind==='time-from')state.time.from_ns=Math.min(value,state.time.to_ns-1);else if(drag.kind==='time-to')state.time.to_ns=Math.max(value,state.time.from_ns+1);else{const shift=Math.max(bounds.from-drag.from,Math.min(bounds.to-drag.to,delta));state.time.from_ns=drag.from+shift;state.time.to_ns=drag.to+shift;}updateWindow();}; svg.onpointerup=event=>{if(drag&&drag.pointerId===event.pointerId){drag=null;svg.releasePointerCapture(event.pointerId);render();}};svg.onpointercancel=svg.onpointerup;
  ...
  function paintHistogram(result) {
    const all=result.all,svg=id('histogram'),width=720,height=126,bins=30;svg.innerHTML='';histogramView=null;if(!all.length)return;const durations=result.durations||all.map(invocation=>invocation.end_ns-invocation.start_ns),min=Math.min(...durations),max=Math.max(...durations),span=Math.max(1,max-min),counts=Array(bins).fill(0),bucketByIndex=[];durations.forEach((duration,index)=>{const bucket=Math.min(bins-1,Math.floor((duration-min)/span*bins));counts[bucket]++;bucketByIndex[index]=bucket;});const peak=Math.max(1,...counts),bars=[];counts.forEach((value,index)=>{const x=index*width/bins,h=value/peak*92,bar=svgEl('rect',{class:'hist-bar',x:x+1,y:98-h,width:width/bins-2,height:h,rx:2});bars.push(bar);svg.appendChild(bar);});const xFor=value=>Math.max(0,Math.min(width,(value-min)/span*width)),windowRect=svgEl('rect',{class:'hist-window',x:0,y:0,width:2,height:103}),lowLine=svgEl('line',{class:'hist-handle','data-edge':'low',y1:0,y2:102}),highLine=svgEl('line',{class:'hist-handle','data-edge':'high',y1:0,y2:102}),lowHit=svgEl('rect',{class:'hist-handle-hit','data-edge':'low',y:0,width:16,height:103}),highHit=svgEl('rect',{class:'hist-handle-hit','data-edge':'high',y:0,width:16,height:103});svg.appendChild(windowRect);svg.appendChild(lowLine);svg.appendChild(highLine);svg.appendChild(lowHit);svg.appendChild(highHit);const label=(text,x)=>{const node=svgEl('text',{class:'hist-label',x,y:119});node.textContent=text;svg.appendChild(node);};label(ns(min),0);label(ns(max),width-66);
    const valueAt=(event,rect)=>min+Math.max(0,Math.min(1,(event.clientX-rect.left)/rect.width))*span; const update=()=>{const low=state.latency.low_ns,high=state.latency.high_ns,lowX=xFor(low),highX=xFor(high),range=rankBounds(all.length,state.percentile),selected=Array(bins).fill(false);for(let index=range[0];index<range[1];index++)selected[bucketByIndex[index]]=true;bars.forEach((bar,index)=>bar.setAttribute('class',selected[index]?'hist-selected':'hist-bar'));windowRect.setAttribute('x',lowX);windowRect.setAttribute('width',Math.max(2,highX-lowX));lowLine.setAttribute('x1',lowX);lowLine.setAttribute('x2',lowX);highLine.setAttribute('x1',highX);highLine.setAttribute('x2',highX);lowHit.setAttribute('x',lowX-8);highHit.setAttribute('x',highX-8);id('pct-low').value=state.percentile.low;id('pct-high').value=state.percentile.high;};histogramView={all,durations,min,max,span,svg,width,update};
    const begin=(kind,event)=>{const rect=svg.getBoundingClientRect(),next=valueAt(event,rect);if(kind==='move'&&(next<state.latency.low_ns||next>state.latency.high_ns))return;const edgeValue=kind==='low'?state.latency.low_ns:state.latency.high_ns;drag={kind:kind==='move'?'latency-move':`latency-${kind}`,edge:kind,pointerId:event.pointerId,rect,startDuration:next,low:state.latency.low_ns,high:state.latency.high_ns,offset:next-state.latency.low_ns,edgeOffset:next-edgeValue};svg.setPointerCapture(event.pointerId);event.preventDefault();};lowHit.addEventListener('pointerdown',event=>{event.stopPropagation();begin('low',event);});highHit.addEventListener('pointerdown',event=>{event.stopPropagation();begin('high',event);});svg.onpointerdown=event=>{if(event.target.tagName!=='text'&&!event.target.classList.contains('hist-handle-hit'))begin('move',event);};svg.onpointermove=event=>{if(!drag||drag.pointerId!==event.pointerId||!drag.kind.startsWith('latency-'))return;const next=valueAt(event,drag.rect);if(drag.kind==='latency-low')state.latency.low_ns=Math.min(next-drag.edgeOffset,state.latency.high_ns);else if(drag.kind==='latency-high')state.latency.high_ns=Math.max(next-drag.edgeOffset,state.latency.low_ns);else{const size=drag.high-drag.low,low=Math.max(min,Math.min(max-size,next-drag.offset));state.latency.low_ns=low;state.latency.high_ns=low+size;}syncLatencyWindow(durations);histogramView.update();};svg.onpointerup=event=>{if(drag&&drag.pointerId===event.pointerId){drag=null;svg.releasePointerCapture(event.pointerId);render();}};svg.onpointercancel=svg.onpointerup;update();
  }
  ...
  function showFrameTooltip(event,current) { const tooltip=id('flame-tooltip'); tooltip.innerHTML=`<strong>${escapeHtml(current.name)}</strong><span>Inclusive: ${ns(current.value)} · Self: ${ns(current.selfValue)}</span><span>Samples: ${count(current.sampleCount)} · Invocations: ${count(current.invocations.size)}</span><span>CPU: ${ns(current.cpu)} · Off-CPU: ${ns(current.off)}</span>`; tooltip.hidden=false; tooltip.style.left=`${Math.min(window.innerWidth-350,event.clientX+14)}px`; tooltip.style.top=`${Math.min(window.innerHeight-145,event.clientY+14)}px`; }
...
mod tests {
    ...
    fn report_is_one_offline_file_with_timeline_and_frame_details() {
        ...
        assert!(html.contains(
            "Invocation bars and sampled work are shown beneath the selected time window."
        ));
        ...
        assert!(html.contains("rect:svg.getBoundingClientRect()"));

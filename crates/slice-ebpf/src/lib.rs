...
const PERF_FLAG_FREQUENCY: u64 = 1 << 10;
...
/// Attach to an already-running process and collect a profile for the requested
/// duration. The target process itself is never modified or stopped.
pub fn capture_pid(request: &CaptureRequest) -> Result<Profile> {
    ...
    let skel = open
        .load()
        .context("loading Slice BPF program; check CAP_BPF and kernel support")?;
    ...
    let ring = ring.build().context("building BPF ring buffer")?;
    ...
    while Instant::now() < deadline && process_exists(request.pid) {
        ring.poll(Duration::from_millis(100))
            .context("polling BPF ring buffer")?;
    }
    drop(ring);
    drop(perf_links);
    drop(entry_link);
...
fn rebuild_threads(profile: &mut Profile) {
...
fn open_sampling_event(cpu: usize) -> Result<RawFd> {
    let attr = PerfEventAttr {
        ...
        sample_period: SAMPLE_FREQUENCY_HZ,
        sample_type: 0,
        read_format: 0,
        flags: PERF_FLAG_FREQUENCY | (1 << 0),
        wakeup: 1,
        bp_type: 0,
        config1: 0,
        config2: 0,
